...
queueMgmt(DLQLimit, HBQ, DLQ, ClientList, ClientLifetime) ->
  receive
    ...
    {dropmessages, {Message, Number}} -> dropMessageInHBQ({Number, Message}, HBQ, DLQ, ClientList, ClientLifetime, DLQLimit);
...
dropMessageInHBQ(MessageTuple, HBQ, DLQ, ClientList, ClientLifetime, DLQLimit) ->
  ...
  case werkzeug:lengthSL(NewHBQ) > DLQLimit / 2 of
    true -> pushMessagesInDLQ(NewHBQ, DLQ, ClientList, ClientLifetime, DLQLimit, true);
...
clientMgmt_newMessageNumberOfClient(ClientPID, lastMessagenumberSend, HBQ, DLQ, ClientList, ClientLifetime, DLQLimit) ->
  ClientElem = [{Elem, LastMessageNumberSend, Timer} || {Elem, LastMessageNumberSend, Timer} <- ClientList, Elem == ClientPID],
  case ClientElem == [] of
    false ->
